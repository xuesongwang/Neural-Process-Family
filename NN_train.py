...
def main_GP():
    kernel = 'EQ' # EQ or period or matern
    ...
    for epoch in tqdm(range(TRAINING_ITERATIONS)):
        ...
        if (epoch % 100 == 0 and epoch<VAL_AFTER) or  epoch % VAL_AFTER == 0:
            val_loss = validation(dataset, nn)
    ...
    # writer.close()
    print("finished training NP!"+kernel)


def main_realword():
    # define hyper parameters
    dataname = 'NIFTY50'  # EQ or period
    TRAINING_ITERATIONS = int(2e4) # 2e5 is 113 hours!!
    MAX_CONTEXT_POINT = 50
    VAL_AFTER = 1e2
    BEST_LOSS = -np.inf
    ...
    for epoch in tqdm(range(TRAINING_ITERATIONS)):
        ...
        if (epoch % 50 == 0 and epoch < VAL_AFTER) or epoch % VAL_AFTER == 0:
            val_loss = validation(val_loader, nn, test_batch=1, mode="NIFTY")
            # save_plot(epoch, plot_data, cnp)  # save training process, optional
            # writer.add_scalars("Log-likelihood", {"val": val_loss}, epoch)
            if val_loss > BEST_LOSS:
                BEST_LOSS = val_loss
                print("save module at epoch: %d, val log-likelihood: %.4f, training loss:%.4f" %
                      (epoch, val_loss, -loss))
                torch.save(nn.state_dict(), 'saved_model/'+dataname+'_NN.pt')
    # writer.close()
    print("finished training NN! " + dataname)
