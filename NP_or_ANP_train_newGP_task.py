...
from module.utils import compute_loss, comput_kl_loss, to_numpy, load_plot_data, compute_importance_sampling
import torch
...
def save_plot(dataset, kernel, model, max_num_context = 50, max_num_extra_target=50, name='tau', MODELNAME='NP'):
    ...
    (mean, var), _, _ = model(x_context.to(device), y_context.to(device), x_target.to(device),num_samples=15)
    ...
    for i in range(batch_size):
        ...
        ax.plot(to_numpy(x_target_order), to_numpy(mean_order.T), linewidth=2, zorder=1, alpha=min(8.0 / num_samples, 1),
                color=linecolor)
...
def train_model_loader(model, dataloader, optim, mode_name, writer, val_loader = None, start_epoch=0, training_epoch =1, load_prior=False, loss_strategy='IS'):
        ...
        for i, data in enumerate(dataloader):
            ...
            if val_loader is None:
                if i == 0 or (i + 1) % val_every_epoch == 0:
                    writer.add_scalars("Log-likelihood", {"%s"%mode_name: -y_likelihood.item()}, int(max_epoch+i/val_every_epoch))
            else:
                model.eval()
                if i == 0 or (i + 1) % val_every_epoch == 0:
                    if isinstance(val_loader, list):
                        ...
                        if not load_prior:
                            ...
                            writer.add_scalars("Log-likelihood", {"adopting_tau_prior_%s" % mode_name: -eval_loss},
                                               max_epoch + int(i / val_every_epoch) + start_epoch)
        max_epoch += i/val_every_epoch
    return max_epoch



def main_GP(kernel_idx1=0, kernel_idx2=1, MODELNAME = 'NP', training_epoch=1, new_epoch=1, loss_strategy='IS'):
    # set up tensorboard
    ...
    # tau_dataset = SineData(amplitude_range=(-1., 1.), shift_range=(-.5, .5), num_samples=n_samples)
    # dataset_tr_k1 = SineData(amplitude_range=(-1., 1.), shift_range=(-.5, .5), num_samples=n_samples)
    # mixed_tr = dataset_tr_k1
    # dataset_val_k1 = SineData(amplitude_range=(-1., 1.), shift_range=(-.5, .5), num_samples=int(1000*scale))
    # dataset_val_k2 = dataset_val_k1
    ...
    train_k2_loader = torch.utils.data.DataLoader(dataset_tr_k2, batch_size=batch_size, shuffle=True,
    ...
    training_epoch = training_epoch if torch.cuda.is_available() else 50
    star_epoch = train_model_loader(np, tau_loader, optim, kernel_1, writer, training_epoch=training_epoch, loss_strategy = loss_strategy)
    # save model output
    ...
    ######## SETTING 2: Ineffective Prior #################
    # star_epoch = 15620
    # step 2: train on kernel 2 completely from scratch
    # optim = torch.optim.Adam(np.parameters(), lr=3e-4, weight_decay=1e-5)
    # train_model_loader(np, train_k2_loader, optim, kernel_2, writer,
    #                    val_loader=val_k2_loader,
    #                    start_epoch=star_epoch, training_epoch=new_epoch)
    # torch.save(np.state_dict(), 'saved_model/' + '%s_from_scratch' % kernel_2 + '_' + MODELNAME + '.pt')
    # np.load_state_dict(torch.load('saved_model/' + '%s_from_scratch'%kernel_2 + '_' + MODELNAME + '.pt', map_location=device))
    # save_plot(dataset_val_k2, kernel_1, np, name='%s_from_scratch' % kernel_2, MODELNAME=MODELNAME)
    # writer.close()
    # print("finished training!")


    # step 1: load the model from the prior and further train it using kernel-2
    # np.load_state_dict(torch.load('saved_model/' + '%s_tau'%kernel_1 + '_' + MODELNAME + '.pt', map_location=device))
    # print("loading the prior from kernel-1!")
    # optim = torch.optim.Adam(np.parameters(), lr=3e-4, weight_decay=1e-5)
    # train_model_loader(np, train_k2_loader, optim, kernel_2, writer,
    #                    val_loader=val_k2_loader,
    #                    start_epoch=star_epoch, training_epoch=new_epoch, load_prior=True)
    # np.load_state_dict(torch.load('saved_model/' + '%s_from_scratch'%kernel_2 + '_' + MODELNAME + '.pt', map_location=device))
    # save_plot(dataset_val_k2, kernel_1, np, name='%s_from_prior' % kernel_2, MODELNAME=MODELNAME)
...
if __name__ == '__main__':
    ...
    main_GP(kernel_idx1=opt.kernel1, kernel_idx2=opt.kernel2, MODELNAME=opt.modelname, new_epoch=opt.train_k2_epoch, training_epoch=opt.train_tau_epoch, loss_strategy=opt.loss)
    # main_realworld()
