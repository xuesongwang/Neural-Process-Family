...
from module.convCNP import ConvCNP, UNet, to_multiple
...
def testing(data_test, model, test_batch = 64, mode = 'GP'):
    ...
    for i in tqdm(range(test_batch)):
        ...
        num_points = int(to_multiple(model.points_per_unit * (x_max - x_min),
                                     model.multiplier))
        x_all = torch.linspace(x_min, x_max, num_points).to(device)
        x_all = x_all[None, :, None].repeat(x_context.shape[0], 1, 1)
...
def main_GP():
    ...
    convcnp.load_state_dict(torch.load('saved_model/' + kernel + '_' + MODELNAME + '.pt', map_location=device))
