...
def get_gp_dataset(name='RBF', mode='train', n_samples=50000):
    ...
    dataset = GPDataset(kernel=kernel, save_file=save_file, n_samples =n_samples)  if torch.cuda.is_available() else \
    GPDataset(kernel_name = name, kernel=kernel, save_file=save_file, n_samples=100)# local /HPC
    # return dataset
    return dataset
...
def gp_transport_task(kernel1='RBF', kernel2 = 'Periodic_Kernel'):
    scale = 1 if torch.cuda.is_available() else 0.01
