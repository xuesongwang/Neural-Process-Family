...
from module.utils import init_sequential_weights, init_layer_weights, weights_init
...
class DeepKernelLatent(nn.Module):
    def __init__(self, out_channels):
        super(DeepKernelLatent, self).__init__()
        self.in_channels = 1
        self.out_channels = out_channels
        self.attn = CrossAttention(input_dim=self.in_channels, latent_dim=out_channels, values_dim=1, num_heads=4)
        weights_init(self.attn)
    ...
    def forward(self, x, y, t):
        h = self.attn(y, x, t)
        h_mean, h_std = torch.split(h, self.out_channels//2, dim=-1)
        h_std = F.softplus(h_std)
...
class FinalLayer(nn.Module):
    ...
    def build_weight_model(self):
        ...
        """
        model = nn.Sequential(
            nn.Linear(self.in_channels, self.latent_dim),
            nn.ReLU(),
            nn.Linear(self.latent_dim, self.out_channels)
        )
        init_sequential_weights(model)
        return model
...
class Discriminator(nn.Module):
    ...
    def forward(self, y_target):
        # input dim: (bs, n_target, y_dim)
        result = self.discriminator(y_target)
        result = torch.mean(result, dim=1)
        return result

class SCANPV2(nn.Module):
    ...
    def __init__(self, rho, points_per_unit, device = torch.device("cpu")):
        ...
        # Instantiate encoder
        self.mean_deter_encoder = DeepKernel(out_channels=self.rho.in_channels//2)
        self.mean_laten_encoder = DeepKernelLatent(out_channels=self.rho.in_channels) # half for mean, half for std
        # self.sigma_encoder = DeepKernel(out_channels=self.rho.in_channels)
        # self.encoder = DeepKernel(out_channels=self.rho.in_channels)

        # Instantiate mean and standard deviation layers, decoder act as an generator
        self.mean_decoder = FinalLayer(in_channels=self.rho.out_channels,
                                     init_length_scale=init_length_scale)
        self.sigma_decoder = FinalLayer(in_channels=self.rho.out_channels,
                                      init_length_scale=init_length_scale)
    ...
    def forward(self, x, y, x_out, y_out = None):
        ...
        # over the data last.
        h_mean_deter = self.activation(self.mean_deter_encoder(x, y, x_grid))
        h_mean_latent_dist = self.mean_laten_encoder(x, y, x_grid)
        if y_out != None:
            h_mean_latent_dist_target = self.mean_laten_encoder(x_out, y_out, x_grid)
        else:
            h_mean_latent_dist_target = None

        h_mean_latent = h_mean_latent_dist.rsample()

        h = torch.cat([h_mean_deter, h_mean_latent],dim=-1)
        h = h.permute(0, 2, 1)
        h = h.reshape(h.shape[0], h.shape[1], num_points)
        h = self.rho(h)
        h = h.reshape(h.shape[0], h.shape[1], -1).permute(0, 2, 1)
        mean = self.mean_decoder(x_grid, h, x_out)
        # h_var_deter = self.activation(self.sigma_encoder(x, x, x_grid))
        # h_var = h_var_deter.permute(0, 2, 1)
        # h_var = h_var.reshape(h_var.shape[0], h_var.shape[1], num_points)
        # h_var = self.rho(h_var)
        # h_var = h.reshape(h_var.shape[0], h_var.shape[1], -1).permute(0, 2, 1)
        sigma = self.sigma_fn(self.sigma_decoder(x_grid, h, x_out))

        # denorm back to the original space
        # mean, sigma = self.output_denorm(mean, sigma, y_mean, y_var)
        mean_detach = self.mean_decoder(x_grid, h.detach(), x_out)
        sigma_detach = self.sigma_fn(self.sigma_decoder(x_grid, h.detach, x_out))
        return mean, sigma, h_mean_latent_dist, h_mean_latent_dist_target, mean_detach, sigma_detach
