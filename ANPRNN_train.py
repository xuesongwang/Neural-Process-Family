...
def save_plot(epoch, data, model):
    ax, fig = plt.subplots()
    (x_context, y_context), x_target = data.query
    x_grid = torch.arange(-2, 2, 0.01)[None, :, None].repeat([x_context.size(0), 1, 1]).to(device)
...
if __name__ == '__main__':
    ...
    optim = torch.optim.Adam(anprnn.parameters(), lr=3e-4, weight_decay=1e-5)

    for epoch in tqdm(range(TRAINING_ITERATIONS)):
        anprnn.train()
        data = dataset.generate_curves(include_context=True, sort=True)
        (x_context, y_context), x_target = data.query
        # save_plot(epoch, data, anprnn)
        (mean, var), prior, poster = anprnn(x_context.to(device), y_context.to(device), x_target.to(device), data.y_target.to(device))
        ...
        if (epoch % 100 == 0 and epoch<VAL_AFTER) or  epoch % VAL_AFTER == 0:
            val_loss = validation(dataset, anprnn)
            ...
            if val_loss > BEST_LOSS:
                ...
                torch.save(anprnn.state_dict(), 'saved_model/'+kernel+'_' + MODELNAME+'.pt')
    writer.close()
