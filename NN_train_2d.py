...
def validation(data_val, model):
    ...
    for i, (img, _) in tqdm(enumerate(data_val)):
        ...
        mean, var = model(x_context.to(device))
        loss = compute_loss(mean, var, y_context.to(device))
        total_ll += -loss.item()
    return total_ll / (i+1)
...
def validation_meta(data_test, model, VAL_ITER=10):
    ...
    for i, batch in tqdm(enumerate(data_test)):
        ...
        context_mask, target_mask = generate_mask(img)
        x_context, y_context, x_target, y_target = img_mask_to_np_input(img, context_mask, target_mask,include_context=False)
...
def main_nonmeta():
    ...
    optim = torch.optim.Adam(nn.parameters(), lr=3e-4, weight_decay=1e-5)
    ...
    for epoch in tqdm(range(TRAINING_ITERATIONS)):
        for i, (img, _) in tqdm(enumerate(dataset.trainloader)):
            ...
            mean, var = nn(x_context.to(device))
            loss = compute_loss(mean, var, y_context.to(device))
            optim.zero_grad()
            loss.backward()
            optim.step()
            # writer.add_scalars("Image Log-likelihood", {"train":-loss.item()}, epoch)
        val_loss = validation(dataset.valloader, nn)
        ...
        if val_loss > BEST_LOSS:
            ...
            torch.save(nn.state_dict(), 'saved_model/' + kernel + '_NN.pt')
        # writer.close()
    print("finished training NN!" + kernel)
...
def main_meta():
    ...
    for epoch in range(TRAINING_ITERATIONS):
        for i, batch in tqdm(enumerate(dataloader)):
            ...
            loss.backward()
            optim.step()
        val_loss = validation_meta(dataloader, nn)
        if val_loss > BEST_LOSS:
            BEST_LOSS = val_loss
            print("save module at epoch: %d, val log-likelihood: %.4f" % (epoch, val_loss))
            torch.save(nn.state_dict(), 'saved_model/' + data_name + '_NN.pt')
    print("finished training NN!" + data_name)
